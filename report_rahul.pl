...
$cred = `cat /opt/Dashboard/data/test-engineering/project/password/credentials`;
...
foreach $func(@functions)
{
	...
	$executionrate = sprintf "%.0f",($ar->{scriptexecuted}/$ar->{scriptplanned})*100;
			...
			if($func =~ /MMX|PTX|ESBU|BBE|ACX|JUNOS|RPD/)
			{
...
$table .= "<center><table style='hieght:100%;width:100%' class='table table-bordered table-striped'><thead><tr><th colspan=2>Regression Phases</th><th>Matrices</th>";
...
foreach $func(@functions)
{
	...
	if($func =~ /MMX|PTX|ACX|BBE|ESBU|JUNOS|RPD/ )
	{
...
foreach $func(@functions)
{
	...
	if($func =~ /JUNOS SW/)  { $disp = "Kernel/Mgmt";} 
        if($func =~ /QFX10008/)  { $disp = "QFX10008/<br>QFX10016";}
	if($func !~ /MMX|PTX|ESBU|JUNOS|ACX|BBE|RPD/)
	{
	$table .= "<th><a class='headerlink' id='$funccount' href='javascript:getdetailedreport(\"$releasestr\",\"$func\");'>$disp<a/></th>";
	$planned .= "<td class='$colclass'>$hash{$func}{scriptplanned}</td>";
	$executed .= "<td class='$colclass'>$hash{$func}{scriptexecuted}</td>";
...
$category = "";
	...
	$newprbuff .= $newbuff;
	### FOR RBU Only
	if($func =~ /MMX|ACX|PTX|BBE|ESBU|JUNOS|RPD/)
	{
