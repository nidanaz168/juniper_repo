...
#<td id=td2><input id=p15 type=button value="RPT_Regress_KM_X56.21_151" onclick="rm('1120735 1120736','rmv2')"></td>
...
#open ("FDT>,respin.$$");
$fdt_contents = `cat respin.$$`;
#$fdt_contents =~s/^\s+|\s+$/</g;
$fdt_contents =~s/td>\s+$//g;
$fdt_contents =~s/<^\s+<td>//g;
$fdt_contents =~s/td>\n/td>/g;
$fdt_contents =~s/\n<td>/<td>/g;
$fdt_pending=0;
...
if (@fdt_contents=~ /fdt|FDT/){
    
foreach $fdt_con(@fdt_contents){


if ($fdt_con =~/button value\=.*fdt|FDT.*/){
$fdt_pending =$fdt_con =~/<td id=td3>(.*)<\/td>/g;
print "fdt pending :$fdt_pending\n";
