...
use lib qw(/usr/local/share/perl/5.22.1/);
use lib qw(/volume/perl/5.8.9/lib/site_perl/5.8.9/);
#use lib qw(/homes/nidanaz/);
use SysTest::DB;
use Storable;
use Data::Dumper;
use LWP::Simple;
...
my $dsn1 = "dbi:Pg:database=milestones;host=$hostname1;port=$port";
...
$cred = `cat /var/www/html/CI_Report/password`;
...
foreach $debugid(@res_id)
{
    ...
    my $recipients='nidanaz@juniper.net';
     $fil_len= `[ -s out.$$ ] && echo "fullfile" || echo "emptyfile"`;
     print "$fil_len\n";
     if($fil_len =~/empty/){
         $url_quote = quotemeta($url);
         `mail -s "Output came blank for $url_quote " "$recipients" <<EOF
           hello
           world
           $url_quote EOF`;
    exit;    
...
sub getTestIdFromScriptName
{
	...
	my $dbh1 = DBI->connect($dsn,$user,$pass)
                or die "Couldn't connect to database ".DBI->errstr;
...
foreach $func(@functions)
{
	...
	$executionrate = sprintf "%.0f",($ar->{scriptexecuted}/$ar->{scriptplanned})*100;
			...
			if($func =~ /MMX|PTX|ESBU|BBE|ACX|JUNOS|RPD|ROUTING NPIs|ROUTING VIRTUAL|Platform-PFE|Platform|PFE|Protocols/)
			{
...
$releasestr =~ s/\./-/g;
open(F,">/var/www/html/CI_Report/data/$releasestr"."_table") or warn "not able to open file";
my $releasestr_table =  "$releasestr"."_table";
`chmod 755 /var/www/html/CI_Report/data/$releasestr_table`;
print "/var/www/html/CI_Report/data/$releasestr"."_table";
### updated time
# Get the Last refreshed time
    $final = "/var/www/html/CI_Report/data/$releasestr"."_table";
...
foreach $func(@functions)
{
	...
	if($func =~ /MMX|PTX|ACX|BBE|ESBU|JUNOS|RPD|ROUTING NPIs|ROUTING VIRTUAL|Platform-PFE|Platform|PFE|Protocols/ )
    {
...
foreach $func(@functions)
{
	...
	if($func =~ /JUNOS SW/)  { $disp = "Kernel<br>Mgmt";} 
        if($func =~ /QFX10008/)  { $disp = "QFX10008/<br>QFX10016";}
	if($func =~ /brackla/i)  { $disp = "$func";}
	if($func !~ /MMX|PTX|ESBU|JUNOS|ACX|BBE|RPD|ROUTING NPIs|ROUTING VIRTUAL|Platform-PFE|Platform|PFE|Protocols/)
	{
	$table .= "<th><a class='headerlink' id='$funccount' href='javascript:getdetailedreport(\"$releasestr\",\"$func\");'>$disp<a/></th>";
	$planned .= "<td class='$colclass'>$hash{$func}{scriptplanned}</td>";
	$executed .= "<td class='$colclass'>$hash{$func}{scriptexecuted}</td>";
...
$function_strip=~ s/\//-/g;
open(F,">/var/www/html/CI_Report/data/$releasestr"."_".$function_strip."_failbreakdown_table");
print F $buff;
close F;
...
open(F,">/var/www/html/CI_Report/data/$releasestr"."_".$function_strip."_prs_table");
...
$dataset1 .= "</dataset>";
$dataset2 .= "</dataset>";
$template = `cat /var/www/html/CI_Report/graph.template`;
$template =~ s/<main>/Raw Pass Rate/g;
$template =~ s/<categories-data>/$category/g;
$template =~ s/<blocker-data>/$dataset/g;
open(F,">/var/www/html/CI_Report/data/$releasestr"."_firstpass");
print F $template;
close(F);
$template = `cat /var/www/html/CI_Report/graph.template`;
...
open(F,">/var/www/html/CI_Report/data/$releasestr"."_pass");
print F $template;
close(F);
...
$category = "";
	...
	$template = `cat /var/www/html/CI_Report/progress_graph.template`;
	$template =~ s/<categories-data>/$category/g;
	$head = $func." Progress";
	...
	$fname = $func;
	$fname =~ s/\//-/g;
	open(F,">/var/www/html/CI_Report/data/$releasestr"."_".$fname."_graph");
	print F $template;
	close(F);
	}
	...
	$function_strip = $func;
	$function_strip=~ s/\//-/g;
	$file = "\"/var/www/html/CI_Report/data/$releasestr"."_".$function_strip."_prs_table\"";
	$prbuff = `cat $file`;
	...
	$newprbuff .= $newbuff;
	### FOR RBU Only
	if($func =~ /MMX|ACX|PTX|BBE|ESBU|JUNOS|RPD|ROUTING NPIs|ROUTING VIRTUAL|Platform-PFE|Platform|PFE|Protocols/)
	{
...
$buff .= "</tbody></table>";
open(F,">/var/www/html/CI_Report/data/$releasestr"."_ALL_failbreakdown_table");
print F $buff;
close(F);
...
open(F,">/var/www/html/CI_Report/data/$releasestr"."_RBU_failbreakdown_table");
print F $rbubreak;
close(F);
...
open(F,">/var/www/html/CI_Report/data/$releasestr"."_ALL_prs_table");
...
open(F,">/var/www/html/CI_Report/data/$releasestr"."_RBU_prs_table");
...
open(F,">/var/www/html/CI_Report/data/$releasestr"."debuglinks");
print F $rbubreak;
close(F);
#print F $rbuopenblockerprbuff.$rbunewprbuff.$rbuallprbuff;
close(F);
...
`chmod -R  777 /var/www/html/CI_Report/data/`;  
