...
var c=$(table.tBodies[0].rows[i]),cols=[];if(c.hasClass(table.config.cssChildRow)){
...
if(table.config.debug){var appendTime=new Date()}var c=cache,r=c.row,n=c.normalized,totalRows=n.length,checkCell=(n[0].length-1),tableBody=$(table.tBodies[0]),rows=[];
for(var i=0;i<totalRows;i++){
...
for(var i=0;i<trs.length;i++){var cells=trs[i].cells;
...
function applyWidget(table){var c=table.config.widgets;var l=c.length;
for(var i=0;i<l;i++){
...
return widgets[i];}}};
function formatSortingOrder(v){if(typeof(v)!="Number"){
