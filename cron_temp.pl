...
$query = " select release from releases where active='yes' $cond;";   #### get the Active releases
    ...
    foreach my $ar(@{$r})
    {
		...
		foreach my $ar1(@{$r1})
		{
				...
				print "./fdt_report.pl $ar1->{drnames} $ar1->{plannedrelease} $ar1->{releasename}  \'$ar1->{function}\'\n";
				$data = `./fdt_report.pl $ar1->{drnames} $ar1->{plannedrelease} $ar1->{releasename}  \'$ar1->{function}\'`;
		                print $data;
                }
	}
$buffer .= "</li></div>";
