...
foreach my $ar(@{$r}){
    $f = $ar->{function};
    push(@functions, $f);
    if($f =~ /^EX2300$|^EX3400$|^EX-Mojito$|^EX-Rodnik$|^V44$|^B54$/i){ $f = "EX"; }
    elsif($f =~ /^QFX-LAVC$|^Qfabric$|^OCX$/i){ $f = "QFX_Legacy"; }
    elsif($f =~ /^EX4600$|^QFX10002$|^QFX10008\/QFX10016$|^QFX-OPUS$|^QFX5200$|^NFX250$/i){ $f = "QFX"; }
    $fns{$f} = 1;
}
...
foreach $func(@functions)
{
	...
	$executionrate = sprintf "%.0f",($ar->{scriptexecuted}/$ar->{scriptplanned})*100;
        ...
        $func1 = $func;
    if($func =~ /^EX2300$|^EX3400$|^EX-Mojito$|^EX-Rodnik$|^V44$|^B54$/i){ $func1 = "EX"; }
    elsif($func =~ /^QFX-LAVC$|^Qfabric$|^OCX$/i){ $func1 = "QFX_Legacy"; }
    elsif($func =~ /^EX4600$|^QFX10002$|^QFX10008\/QFX10016$|^QFX-OPUS$|^QFX5200$|^NFX250$/i){ $func1 = "QFX"; }

			$hash{$func1}{scriptplanned} += $ar->{scriptplanned};
			$hash{$func1}{scriptexecuted} += $ar->{scriptexecuted};
			$hash{$func1}{tcexecuted} += $ar->{tcexecuted};
			$hash{$func1}{firstrunpassrate} += $ar->{firstrunpassrate};
			$hash{$func1}{overallpassrate} += $ar->{overallpassrate};
			$hash{$func1}{tcpassrate} += $ar->{tcpassrate};
			$hash{$func1}{tcfailrate} += $ar->{tcfailrate};
			$hash{$func1}{plannedpassrate} += $ar->{plannedpassrate};
			$hash{$func1}{overallfailrate} += $ar->{overallfailrate};
			$hash{$func1}{debugstart} = $ar->{debugstart};
			$hash{$func1}{debugend} = $ar->{debugend};
			$hash{$func1}{newprs} += $ar->{newprs};
			$hash{$func1}{allprs} += $ar->{allprs};
			$hash{$func1}{blockerprs} += $ar->{blockerprs};
			$hash{$func1}{openblockerprs} += $ar->{openblockerprs};
			$hash{$func1}{tobedebugged} += $ar->{tobedebugged};
			$hash{$func1}{totaldebugcount} += $ar->{totaldebugcount};
			$hash{$func1}{completeddebugcount} += $ar->{completeddebugcount};
			$hash{$func1}{debugper} += $ar->{debugper};
			$hash{$func1}{debugids} .= $ar->{debugids}.",";
			$hash{$func1}{totalscriptfailed} += $ar->{totalscriptfailed};
			$hash{$func1}{totalscriptpassed} += $ar->{totalscriptpassed};
			$hash{$func1}{totaltcfailed} += $ar->{totaltcfailed};
			$hash{$func1}{totaltcpassed} += $ar->{totaltcpassed};
			$hash{$func1}{totalfirstrunpass} += $ar->{totalfirstrunpass};
			$hash{$func1}{totalfirstrunscript} += $ar->{totalfirstrunscript};
			$hash{$func1}{respindebugcount} += $ar->{respindebugcount};
            $hash{$func1}{respincompleteddebug} += $ar->{respincompleteddebug};
            $hash{$func1}{respinpendingdebug} += $ar->{respinpendingdebug};
            $hash{$func1}{respincompleteddebugper} += $ar->{respincompleteddebugper};
			$hash{$func1}{executionrate} += $executionrate;
	...
	if($c ==0 )
	{
			$hash{$func1}{scriptplanned} = 0;
			$hash{$func1}{scriptexecuted} = 0;
			$hash{$func1}{tcexeuted} = 0;
			$hash{$func1}{firstrunpassrate} = 0;
			$hash{$func1}{overallpassrate} = 0;
			$hash{$func1}{tcpassrate} = 0;
			$hash{$func1}{tcfailrate} = 0;
			$hash{$func1}{plannedpassrate} = 0;
			$hash{$func1}{overallfailrate} = 0;
			$hash{$func1}{debugstart} = "";
			$hash{$func1}{debugend} = "";
			$hash{$func1}{newprs} = 0;
			$hash{$func1}{allprs} = 0;
			$hash{$func1}{blockerprs} = 0;
			$hash{$func1}{openblockerprs} = 0;
			$hash{$func1}{tobedebugged} = 0;
			$hash{$func1}{totaldebugcount} = 0;
			$hash{$func1}{debugper} = 0;
			$hash{$func1}{completeddebugcount} = 0;
			$hash{$func1}{totalscriptfailed} = 0;
			$hash{$func1}{totalscriptpassed} = 0;
			$hash{$func1}{totaltcfailed} = 0;
			$hash{$func1}{totaltcpassed} = 0;
			$hash{$func1}{executionreate} = 0;
			$hash{$func1}{totalfirstrunpass} = 0;
			$hash{$func1}{totalfirstrunscript} = 0;
			$hash{$func1}{respindebugcount} = 0;
            $hash{$func1}{respincompleteddebug} = 0;
            $hash{$func1}{respinpendingdebug} = 0;
            $hash{$func1}{respincompleteddebugper} = 0;
	}
}
$breakcolcount+=8;
foreach $func1(keys %hash){
        if($hash{$func1}{totalscriptpassed} > 0)
        {
                $hash{$func1}{overallpassrate} = sprintf "%.2f",($hash{$func1}{totalscriptpassed}/$hash{$func1}{scriptexecuted}) * 100;
        }
        if($hash{$func1}{totalscriptpassed} > 0)
        {
                $hash{$func1}{plannedpassrate} = sprintf "%.2f",($hash{$func1}{totalscriptpassed}/$hash{$func1}{scriptplanned}) * 100;
        }
        if($hash{$func1}{totalscriptfailed} > 0)
        {
                $hash{$func1}{overallfailrate} = sprintf "%.2f",($hash{$func1}{totalscriptfailed}/$hash{$func1}{scriptexecuted}) * 100;
        }
        if($hash{$func1}{totaltcpassed} > 0)
        {
                $hash{$func1}{tcpassrate} = sprintf "%.2f",($hash{$func1}{totaltcpassed}/$hash{$func1}{tcexecuted}) * 100;
        }
        if($hash{$func1}{totalscriptpassed} > 0)
        {
                $hash{$func1}{plannedpassrate} = sprintf "%.2f",($hash{$func1}{totalscriptpassed}/$hash{$func1}{scriptplanned}) * 100;
        }
        if($hash{$func1}{totaltcfailed} > 0)
        {
                $hash{$func1}{tcfailrate} = sprintf "%.2f",($hash{$func1}{totaltcfailed}/$hash{$func1}{tcexecuted}) * 100;
        }
        $hash{$func1}{executionrate} = 0;
        if($hash{$func1}{scriptplanned} > 0)
        {
        $hash{$func1}{executionrate} = sprintf "%.0f",($hash{$func1}{scriptexecuted}/$hash{$func1}{scriptplanned}) * 100;
        }
        #$hash{$func1}{firstrunpassrate} = sprintf "%.2f",($hash{$func1}{firstrunpassrate} / $funccount);
        $hash{$func1}{debugper} = 0;
        if($hash{$func1}{totaldebugcount} > 0)
        {
        $hash{$func1}{debugper} = sprintf "%.0f",($hash{$func1}{completeddebugcount} / $hash{$func1}{totaldebugcount}) * 100;
        }
        $hash{$func1}{firstrunpassrate} = 0;
        if($hash{$func1}{totalfirstrunscript}  > 0)
        {
        $hash{$func1}{firstrunpassrate} = sprintf "%.2f",($hash{$func1}{totalfirstrunpass} / $hash{$func1}{totalfirstrunscript}) * 100;
        }
        if ($hash{$func1}{respindebugcount}>0) {
        $hash{$func1}{respincompleteddebugper} = sprintf "%.2f",($hash{$func1}{respincompleteddebug} / $hash{$func1}{respindebugcount}) * 100;
        } else {
        $hash{$func1}{respincompleteddebugper} = 0.00;
        }
        $hash{$func1}{totaldebugrate} = 0;
        if($hash{$func1}{scriptexecuted} > 0)
        {
        $hash{$func1}{totaldebugrate} = sprintf "%.2f",($hash{$func1}{tobedebugged} / $hash{$func1}{scriptexecuted}) * 100;
        }
                $hash{$func1}{rbudebugrate} = 0;
        if($hash{$func1}{scriptexecuted} > 0)
        {
                $hash{$func1}{rbudebugrate} = sprintf "%.2f",($hash{$func1}{tobedebugged} / $hash{$func1}{scriptexecuted}) * 100;
        }
...
$query = "insert into daily_regression_progress values('$releasestr','ALL','$today',$hash{total}{executionrate},$hash{total}{overallpassrate},$hash{total}{overallfailrate},0,$hash{total}{totaldebugrate})";
$sth=$dbh->prepare($query);
            $sth->execute;
...
$query = "insert into daily_regression_progress values('$releasestr','RBU','$today',$hash{routing}{executionrate},$hash{routing}{overallpassrate},$hash{routing}{overallfailrate},0,$hash{switching}{rbudebugrate})";
$query= "delete from daily_regression_progress where day='$today' and release='$releasestr' and function='SWIT'";
            $sth=$dbh->prepare($query);
            $sth->execute;
$query = "insert into daily_regression_progress values('$releasestr','SWIT','$today',$hash{switching}{executionrate},$hash{switching}{overallpassrate},$hash{switching}{overallfailrate},0,$hash{switching}{rbudebugrate})";
$sth=$dbh->prepare($query);
            $sth->execute;
