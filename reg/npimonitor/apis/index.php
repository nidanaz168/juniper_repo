...
function getSummaryHeaders($relName,$dbh){
	...
	while ($row = pg_fetch_array($result)){
	    ...
	    #$fieldstring["pass"] = "pass$i";
	    #$fieldstring["fail"] = "fail$i";
	    #$fieldstring["notexec"] = "notexec$i";
	...
	foreach ($relstrings as $key => $value){
		...
		for ($v =0; $v < 5 ;$v++) {
    		...
    		$subArray[$n]["headerName"] = "$value1";
    		$subArray[$n]["field"] = "$fieldstring[$z]";
...
function getsummary($relName,$dbh){
	...
	$joinquery = array();
	foreach ($relstring as $key => $value){
		$passquery = "sum (case when $value ~ 'PASS'  then 1 else 0 end) as pass$key";
		$failquery = "sum (case when $value ~ 'Fail'  then 1 else 0 end) as fail$key";
		$noquery   = "sum (case when $value ~ 'Not Executed' then 1 else 0 end) as not_exec$key";
		$nodebug   = "sum (case when $value ~ 'Debugged' then 1 else 0 end) as not_debug$key";
		$na   = "sum (case when $value ~ 'NA' then 1 else 0 end) as na$key";
		array_push($joinquery,$passquery,$failquery,$noquery,$nodebug,$na);
		array_push($fieldval,"pass$key","fail$key","not_exec$key","not_debug$key","na$key");
