...
def task(data):
    # if data[1]=="":
    #     data[1]=data[2]
    command='/var/www/html/CI_Report/report.pl '+ data[0] +" "+data[1]+ " "+data[2] +" "+data[3]
    # print(command)
...
class Regression_DatabaseConnection():
    def __init__(self):
        try:
            self.connection = psycopg2.connect("dbname='regression' user='postgres' host='jdi-reg-tools' password='postgres' port='5432'")
